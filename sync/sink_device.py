...
class SinkDevice:
    ...
    def setup_advertising(self):
        ...
        # Advertising contínuo para múltiplas conexões
        self.advertisement.duration = 0
        logger.info("Advertising contínuo configurado")
    ...
    def _on_packet_received(self, data: bytes, client_address: Optional[str] = None):
        ...
        try:
            ...
            if session_key:
                ...
                logger.warning(
                    f"  Sem session key para {packet.source} - "
            ...
            if self.auth_handler.is_authenticated(client_address):
                ...
                if peer_info and 'session_key' in peer_info:
                    ...
                    if dtls_channel.establish():
                        ...
                        # Re-registar advertising para múltiplas conexões
                        try:
                            ...
                            adv_manager.RegisterAdvertisement(
                                ...
                                {},
                                reply_handler=lambda: logger.info("Advertising re-registado"),
    ...
    def _handle_data_packet(self, packet: Packet, client_address: Optional[str]):
        ...
        if packet.destination == self.my_nid:
            ...
            logger.warning(
                f"  Pacote destinado a {packet.destination} mas Sink não faz forward "
    ...
    def _cleanup_disconnected_client(self, sender: str):
        ...
        if not client_nid:
            logger.warning(f"  Cliente {sender} desconectou mas NID não encontrado")
            return
    ...
    def start_heartbeat_service(self):
        ...
        def send_heartbeat():
            ...
            try:
                ...
                # D-Bus GATT: notificações são broadcast (não suporta unicast)
