...
Implementação de uma rede ad-hoc privada de dispositivos IoT que comunicam exclusivamente via Bluetooth Low Energy (BLE), seguindo a especificação em `docs/project.txt`. 

A rede utiliza topologia em árvore hierárquica, com um Sink (gateway) central e múltiplos nós IoT que funcionam como sensores e routers.
...
- Heartbeat forwarding multi-hop
- Endereçamento baseado em NID (128 bits)
- Heartbeat protocol com assinaturas ECDSA
...
# 2. Gerar certificados (cada dispositivo)
...
node> scan              # Descobrir Sink
node> connect 1         # Conectar ao Sink
node> send Hello!       # Enviar mensagem
```

### Verificar mensagem no Sink
```bash
sink> inbox
```
